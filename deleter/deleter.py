...
class Deleter(commands.Cog):
    ...
    WARNING: This cog has potential API abuse AND SHOULD BE USED CAREFULLY!  If you see any issues arise due to this, please report to bot owner ASAP!"""
    ...
    async def remove(self, ctx, channel: discord.TextChannel, *messages: str):
        ...
        await ctx.send(
            f"Sage has been burned, the negativity is gone, all is good again: {humanize_list(success)}\nMessages that failed to be removed: {humanize_list(failed)}"
        )
