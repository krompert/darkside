...
class Events(commands.Cog):
    def __init__(self, bot: Red):
        self.bot = bot
        self.data = Config.get_conf(self, identifier=654945619651965, force_registration=True)
    ...
    async def MessageWinners(self, guild, winners):
        ...
        for winner in winners:
            try:
                await winner.send(f"You have been chosen for the event on {guild.name}")
            except:
                pass
    ...
    async def _add_roles(self, guild, winners):
        ...
        for winner in winners:
            try:
                ...
                if log_channel:
                    await log_channel.send(f"{winner.mention} was assigned **{role.name}** for being chosen for the event.")
            except:
                pass
        ...
        if data['roles_required']:
            ...
            for role in roles:
                if role in user.roles:
                    try:
                        await user.send("You have entered the registration for the event.")
                    except:
                        pass
                    return
            
            roles= [x.name for x in roles]
            await user.send(f"You must have either one of these roles be register for the event: **{','.join(roles)}**")
            await message.remove_reaction(payload.emoji, user)
    ...
    async def _start(self, ctx):
        ...
        e = discord.Embed()
        e.set_author(name="Event Builder", icon_url=ctx.guild.icon_url)
        e.description = "Name of the event?"
        ...
        e.add_field(name="Event Name", value=response["prize"])
        e.description = "How long should the event registration last for?"
        ...
        try:
            ...
            await self.delete_message(msg)
            return await ctx.send("Timeout, you took too long to respond.")
        e.add_field(name="Registration Duration", value=response["duration"][1])
        e.description = "How many users should be selected for the event?"
        await msg.edit(embed=e)
        def check(m):
        ...
        try:
            ...
            await self.delete_message(msg)
            return await ctx.send("Timeout, you took too long to respond.")
        e.add_field(name="Total Spots", value=response["winners"])
        e.description = "Would you like to limit this event registration to specific roles?\nRespond with role ids, names or mentions or just `no`"
        await msg.edit(embed=e)
        async def check(m):
    ...
    @_event.command(name="end")
    async def _end(self, ctx, messageID: int):
        """End an event registration."""
        data = await self.data.guild(ctx.guild).giveaways()
        if not data or str(messageID) not in data:
            return await ctx.send(f"No event registration was found with the message id: `{messageID}`")
        
        data = await self.data.guild(ctx.guild).giveaways.get_raw(messageID)
        if data['ended']:
            return await ctx.send("This event registration was ended.")
    ...
    @_event.command(name="reroll")
    async def _reroll(self, ctx, messageID: int):
        """Reroll the event participants."""
        data = await self.data.guild(ctx.guild).giveaways()
        if not data or str(messageID) not in data:
            return await ctx.send(f"No event registration was found with the message id: `{messageID}`")
        ...
        data = await self.data.guild(ctx.guild).giveaways.get_raw(messageID)
        if not data['ended']:
            return await ctx.send("This event registration hasn't ended yet.")

        winner = await self.end_giveaway(messageID, data, winners)
        await ctx.send(f"🗡️ The new selected participant is {winner.mention}! Congratulations!")

    async def winners_message(self, winners):
        message = "**Winner:** No one registered for the event."

        if len(winners) == 1:
            for winner in winners:
                message = f"**Selected Participants:** {winner}"

        if len(winners) > 1:
            message = f"**Selected Participants:** {', '.join(winners)}"

        return message

    @_event.group(name="role", invoke_without_command=True)
    async def _role(self, ctx):
        """Manage the roles required to enter the event."""

    @_role.command(name="remove")
    async def _remove(self, ctx, messageID, role: discord.Role):
        """Remove a role from the event registration."""
        data = await self.data.guild(ctx.guild).giveaways()
        if not data or str(messageID) not in data:
            return await ctx.send(f"No event registration was found with the message id: `{messageID}`")
    ...
    async def _add(self, ctx, messageID, role:discord.Role):
        """Add a role for the event registration."""
        ...
        roles = [ctx.guild.get_role(role).mention for role in roles if ctx.guild.get_role(role)]
        await self.embed_msg(messageID, data, None, roles)
        await ctx.send(f"Users with {role.name} will now be allowed to register for the event.")
        
    @_role.command(name="list")
    async def _list(self, ctx, messageID):
        """List all the allowed roles for event registration."""
        data = await self.data.guild(ctx.guild).giveaways()
        if not data or str(messageID) not in data:
            return await ctx.send(f"No event event registration was found with the message id: `{messageID}`")
        ...
        roles = data['roles_required']
        if not roles:
            return await ctx.send("There were no roles allowed for this event registration.")

        roles = [ctx.guild.get_role(role).mention for role in roles if ctx.guild.get_role(role)]
        await ctx.send(embed=discord.Embed(description=",".join(roles), title="Roles Allowed"))

    @_event.group(name="winners", invoke_without_command=True)
    async def _winners(self, ctx):
        """ View the winners of an event."""

    @_winners.command(name="set")
    async def _set(self, ctx, messageID: int, winners: int):
        """ Set the amount of participants for event registration."""
        data = await self.data.guild(ctx.guild).giveaways()
        if not data or str(messageID) not in data:
            return await ctx.send(f"No event registration was found with the message id: `{messageID}`")
        ...
        if data['winners'] == winners:
            return await ctx.send("Previous event participants count is the same as the new one.")
        ...
        await ctx.send(f"Changed the total amount of participants for the event to: **{winners}**.")
    ...
    @commands.group()
    async def eventwinner(self, ctx):
        """Setup autorole for event participants and the event channel."""
        if ctx.invoked_subcommand is None:
            pass

    @eventwinner.command(name="role")
    async def ___role(self, ctx, role: discord.Role=None):
        """Setup autorole for event participants."""
        if not role:
            await self.data.guild(ctx.guild).giveawayRole.set(None)
            await ctx.send("Event participants role has been reset!")
        if role:
            await self.data.guild(ctx.guild).giveawayRole.set(role.id)
            await ctx.send(f"Event participants role has been set to **{role.name}**!")
